...
function buildUpsertFilter(movie: MovieSeed) {
...
export async function upsertMoviesToMongo(params: {
  mongoUri: string;
  dbName: string;
  ...
  try {
    ...
    const collection = db.collection<MovieSeed>(collectionName);

    // Best-effort unique index to prevent duplicates if the collection
    // is mostly clean already.
    try {
      await collection.createIndex(
        { source: 1, title: 1, year: 1 },
        { unique: true, sparse: true, name: "uniq_source_title_year" }
      );
    } catch (e) {
      console.warn(
        `Unique index creation failed (continuing without it): ${String(e)}`
      );
      await collection.createIndex(
        { source: 1, title: 1, year: 1 },
        { unique: false, name: "idx_source_title_year" }
      );
    }
